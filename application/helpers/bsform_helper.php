...
function bs_inputfield_id($display_name, $fieldname, $defaultvalue, $mandatory = false, $printlabel = true, $input_style = '')
{
	$for="";
	$error = form_error($fieldname, '<label class="control-label" style="width:400px;"  for="inputError3">', '</label>');
	if ($error != '')
	{
		$for = "inputError3";
		...
		echo "<div class=\"form-group\" id=\"add_emp_form\">";
	...
	echo "<input type=\"text\" style=\"width:400px;\" class=\"form-control\" for=\"$for\" id=\"$fieldname\" name=\"$fieldname\" value=\"".set_value($fieldname, $defaultvalue)."\" $input_style />";
...
{
	$for="";
    $error = form_error($fieldname, '<label class="control-label" for="inputError3">', '</label>');
    if ($error != '')
    {
    	$for = "inputError3";
        echo "<div class=\"form-group has-error\">";
