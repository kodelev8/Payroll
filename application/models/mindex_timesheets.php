...
class  mindex_timesheets extends CI_Model
{
	 ...
	 * This function ...
	 */
	function timesheets_view($TS_per_page=0,$TS_row=0,$TS_total=0)
	{
		$this->dbAll = $this->load->database('default', TRUE);
  		$query = $this->dbAll->query("select top $TS_per_page * from vw_Timesheets where TS_row between $TS_row and $TS_total and ts_deleted = 0 and  ts_obselete = 0 order by TS_Hours_In desc"); 	
  		return $query->result(); 
	}
	function temptimesheets_view($TS_per_page=0,$TS_row=0,$TS_total=0)
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("select top $TS_per_page * from vw_TempTimesheets where TS_row between $TS_row and $TS_total and ts_deleted = 0 and  ts_obselete = 0 order by TS_Emp_No,TS_Hours_In asc");
		return $query->result();
	}
	function processtimesheets_view($TS_date="")
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("exec [sp_processDTRCheck] '$TS_date' ");
		
// 		echo "exec [sp_processDTRCheck] '$TS_date' ";
// 		return $query->result();
	}
	
	function createtimesheets_view($TS_date="")
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("exec [dbo].[sp_processDTR] '$TS_date' ");
	...
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("select COUNT(*) as count_timesheets from vw_Timesheets  where ts_deleted = 0 and ts_obselete =0");
		return $query->row();
	}
	function check_timesheets_process($ts_date_from=0,$ts_date_to=0)
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("select * from vw_Timesheets where ts_date
				between cast('$ts_date_from' as datetime) and CAST('$ts_date_to' as datetime) and TS_Deleted = 0 and TS_Obselete = 0");
		return $query->result();
	}
	function get_timesheets_emp_no()
	{
		$this->dbAll = $this->load->database('default', TRUE);
	...
	function timesheets_process($ts_emp_no = "",$ts_date_from = "",$ts_date_to = "" )
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("exec Sp_createpayslipsrecord '$ts_emp_no','$ts_date_from','$ts_date_to' ");
		// 		return $query->result();
	}
	function timesheets_num_rows()
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("select [No of Rows] NumRows ,  * from [rows per page] ");
		return $query->result();
	}
	function active_timesheets_num_rows()
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("select [No of Rows] NumRows , 	* from [rows per page] where active =1 ");
		return $query->row();
	}
	function update_timesheets_num_rows($dtr_per_page=0)
	{
		$this->dbAll = $this->load->database('default', TRUE);
...
// 	{
// 		$this->dbAll = $this->load->database('default', TRUE);
// 		$query = $this->dbAll->query("select top $dtr_per_page * from fnSearchTimesheets('$dtr_search') where ts_row between $dtr_row and $dtr_total and deleted = 0 order by ts_time desc");
// 		return $query->result();
// 	}
