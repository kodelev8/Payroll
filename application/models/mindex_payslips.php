...
// 	}
    ...
    {
    	$this->dbAll = $this->load->database('default', TRUE);
    	$query = $this->dbAll->query("select  * from configuration where [key] = '$key' ");
    	return $query->row();
    }
	function payslips_view($emp_position="",$StartDate="",$EndDate="")
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("select  * from vw_payslips where left(Ps_Emp_No,1) in ($emp_position) and Ps_Date_From between '$StartDate' and '$EndDate' and ps_deleted =0 order by  Ps_Emp_No");
		return $query->result();
// echo "select  * from vw_payslips where left(Ps_Emp_No,1) in ($emp_position) and Ps_Date_From between '$StartDate' and '$EndDate' order by  Ps_Emp_No";
	}
	...
	function get_deduction_ca($deduct_emp_no =0,$deduct_date_from= "",$deduct_date_to ="")
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("SELECT * FROM  vwdeduction where  Deduction_Emp_No ='$deduct_emp_no' and Deduction_Date between '$deduct_date_from' and '$deduct_date_to' and deduction_description like '%ca%'");
		return $query->result();
	}
	function get_deduction_uni($deduct_emp_no =0,$deduct_date_from= "",$deduct_date_to ="")
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("SELECT * FROM  vwdeduction where  Deduction_Emp_No ='$deduct_emp_no' and Deduction_Date between '$deduct_date_from' and '$deduct_date_to' and deduction_description like '%uniform%'");
		return $query->result();
	}
...
// 		echo "select top $payslip_per_page * from fnSearchpayslips('$payslip_search') where payslip_row between $payslip_row and $payslip_total and ps_deleted = 0 order by Ps_Date_From desc";
	...
	function count_ts($emp_no = '',$StartDate='',$EndDate='')
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query(" select Count(*) count from vw_Timesheets_temp
				where TS_Emp_No = '$emp_no' and TS_Date between  '$StartDate' and  '$EndDate'  ");
		return $query->row();
	}
	function list_dates_from_ts1($emp_no = '',$StartDate='',$EndDate='')
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query(" exec [sp_listdate_fromtempts] '$emp_no',  '$StartDate','$EndDate'");
		return $query->result_array();
	}
	...
	function temp_process_dtr_to_ts($ts_date='',$ts_emp_no='')
	{
		$ts_date= date("Ymd", strtotime($ts_date));
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("exec [sp_processDTR_temp] '$ts_date','$ts_emp_no'"); 
	}
	function insert_timehsheet($ts_emp_no='',$ts_date='')
	{
		$ts_date= date("Ymd", strtotime($ts_date));
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("exec [sp_inserttimesheet] '$ts_emp_no','$ts_date'");
	}
	...
	{
		$this->dbAll = $this->load->database('default', TRUE);
		$query = $this->dbAll->query("select * from [fnGetPosition]() order by position asc");
		return $query->result();
