...
// 		Settings::loadConfig();
	...
	public function index($PS_row = 0)
	{
		...
		$emp_add = $this->input->post('btn-add');
		$emp_position = $this->input->post('emp_position');
		$StartDate = $this->input->post('wk_start_date');
		$EndDate = $this->input->post('wk_end_date');
		$weekno = date('W') -1;
...
// 		$this->pagination->initialize($config);
		$get_position = $this->mindex_payslips->get_position();
		$data['error_msg']="";
		$data['StartDate']= $StartDate;
		$data['EndDate'] = $EndDate;
		$data['emp_position'] = $emp_position; 
		$data['get_position'] = $get_position;
...
// 		$data['payslips'] = $this->mindex_payslips->payslips_view($PS_per_page,$PS_row,$PS_total);
		$data['payslips'] = $this->mindex_payslips->payslips_view($emp_position,$StartDate,$EndDate);
	...
	public function payslips_print_all( )
	{
		$finalpdf= 'pp_'.uniqid(); 
		$pdffolder = $this->mindex_payslips->get_config('output_pdf_folder');
		$pdffolder = $pdffolder->Value;
		$wordfolder = $this->mindex_payslips->get_config('output_docx_folder');
		$wordfolder = $wordfolder->Value;
		$PS_id =0;
		$foldername= uniqid('payslips'.date('ymdhhmmss'));
		$createfolder = 'md "'.$wordfolder.'/'.$foldername.'"';
		shell_exec($createfolder);
		...
		$deduct_emp_no = $get_payslip_print->Ps_Emp_No;
		$deduct_date_from = $datefrom;
		$deduct_date_to = $dateto;
		$get_deduction_ca = $this->mindex_payslips->get_deduction_ca($deduct_emp_no,$deduct_date_from,$deduct_date_to);
		$get_deduction_uni = $this->mindex_payslips->get_deduction_uni($deduct_emp_no,$deduct_date_from,$deduct_date_to);
// 		echo $get_payslip_print->Ps_Emp_Name;
		if(count($get_deduction_ca) > 0 )
			{
				foreach ($get_deduction_ca as $deduct_cash)
					{
	
				$get_cash_adv= $get_cash_adv+ $deduct_cash->Deduction_Amount;
			}
		...
		if(count($get_deduction_uni)> 0 )
			{
				foreach ($get_deduction_uni as $deduct_uni)
					{
						$get_uniform= $get_uniform+$deduct_uni->Deduction_Amount;
					}
		...
		$templateProcessor->setValue('basepay1', 	number_format((float)$get_payslip_print->Ps_Emp_wages*8, 2, '.', ''));
		$templateProcessor->setValue('allowance1', 	number_format((float)$get_payslip_print->Ps_Add, 2, '.', ''));
		$templateProcessor->setValue('deduction1', 	number_format((float)$get_cash_adv, 2, '.', ''));
		$templateProcessor->setValue('sdeduction1', number_format((float)$get_uniform, 2, '.', ''));
		$templateProcessor->setValue('totalotpay1', number_format((float)$get_payslip_print->Ps_Total_OT_Pay, 2, '.', ''));
		$templateProcessor->setValue('nootdays1', 	number_format((float)$get_payslip_print->Ps_OT_Hours+$get_payslip_print->Ps_Night_Hours, 2, '.', '') /8);
		...
		$templateProcessor->saveAs('templates/temp/'.$foldername.'/'.$file_name );
		
		$sourcedir = '"'.$wordfolder .$foldername.'/'. $file_name.'"';
		
		$outdir = '"'.$pdffolder .$foldername.'/"';
		...
		endforeach;
		$pdftk = 'pdftk '.$pdffolder.$foldername.'/*.pdf cat output '.$pdffolder.$finalpdf.'.pdf';
	 
		shell_exec($pdftk);
		$del_docx = 'RMDIR "'.$wordfolder .$foldername .'" /S /Q';
		$del_pdf = 'RMDIR "'.$pdffolder .$foldername.'" /S /Q';
		 
		$file_name= $finalpdf.'.pdf';
...
// 		$sourcedir = '"C:/xampp/htdocs/payroll/templates/temp/'. $file_name.'"';
// 		$outdir = '"C:/xampp\htdocs/payroll/templates/output/"';
// 		$cmd = 'soffice --headless --convert-to pdf ' .$sourcedir. ' --outdir '.$outdir;
// 		shell_exec($cmd);
// 		echo $cmd;
...
// 		$templateProcessor->setValue('date2', 		 $dateto);
		$templateProcessor->saveAs('templates/temp/'.$file_name );
		$sourcedir = $this->mindex_payslips->get_config('output_docx_folder');
	 	$sourcedir = '"'.$sourcedir->Value. $file_name.'"';
	 	$outdir =  $this->mindex_payslips->get_config('output_pdf_folder');
	 	$outdir = $outdir->Value;
	 	$cmd = 'soffice --headless --convert-to pdf ' .$sourcedir. ' --outdir '.$outdir;
		shell_exec($cmd);
		echo $cmd;
...
// 		$wk_end_date = $this->input->post('hidden_end_date');
		$list_dates=  $this->mindex_payslips->list_dates_from_ts1($ts_emp_no,$wk_start_date,$wk_end_date); 
		$ts_date = "";
		$ts_count = 0;
		$output=0;
		foreach ($list_dates as $Ld)
		{
			...
			$this->mindex_payslips->insert_timehsheet($ts_emp_no,$ts_date);
		}
...
// 		$list_dates_from_ts1=  $this->mindex_payslips->list_dates_from_ts1($emp_no,$StartDate,$EndDate);
		$html ="";
// 		if(count($list_dates_from_ts1)>0)
// 		{
// 			$list_dates =$list_dates_from_ts1;
// 		}
// 		$view_deduction = $this->mindex_deduction->view_deduction($deduction_emp_no,$deduction_date);
...
// 		$StartDate 	= '20180806';
// 		$EndDate 	=  '20180812';
		$count_ts=  $this->mindex_payslips->count_ts($emp_no,$StartDate,$EndDate);
...
// 		$list_dates=  $this->mindex_payslips->list_dates($StartDate,$EndDate);
		...
		$list_dates=  $this->mindex_payslips->list_dates_from_ts1($emp_no,$StartDate,$EndDate);
		$html ="";
...
// 		echo $output;
	...
	public function add_dtr_1()
	{
		...
		$this->mindex_payslips->temp_process_dtr_to_ts($day_login_1,$dtr_emp_no);
		...
		public function add_dtr_2()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_2,$dtr_emp_no);
		...
		public function add_dtr_3()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_3,$dtr_emp_no);
			echo  json_encode($dtr_emp_no) ;
		}
		public function add_dtr_4()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_4,$dtr_emp_no);
		...
		public function add_dtr_5()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_5,$dtr_emp_no);
		...
		public function add_dtr_6()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_6,$dtr_emp_no);
			echo  json_encode($dtr_emp_no) ;
		}
		public function add_dtr_7()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_7,$dtr_emp_no);
			echo  json_encode($dtr_emp_no) ;
		}
		public function add_dtr_8()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_8,$dtr_emp_no);
		...
		public function add_dtr_9()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_9,$dtr_emp_no);
		...
		public function add_dtr_10()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_10,$dtr_emp_no);
		...
		public function add_dtr_11()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_11,$dtr_emp_no);
		...
		public function add_dtr_12()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_12,$dtr_emp_no);
		...
		public function add_dtr_13()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_13,$dtr_emp_no);
		...
		public function add_dtr_14()
		{
			...
			$this->mindex_payslips->temp_process_dtr_to_ts($day_login_14,$dtr_emp_no);
